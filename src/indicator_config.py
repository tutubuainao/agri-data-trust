...
INDICATOR_META: dict[str, dict[str, str | bool]] = {
    "sampling": {
        ...
        "group": "可选质量指标",
        "default": False,
        "applicability": "适合连续采集、有明确时间列、多 sheet 或需要复核采样链路的数据；普通人工 Excel 调查表可不默认启用。",
        "reason": "完整性属于数据质量和采集链路线索，不是最直接的真实性证据，因此默认不启用，并在自定义开启时保持低权重。",
    },
    "smoothness": {
        "label": "平滑度检测",
        "short": "平滑",
        ...
        "reason": "识别过度平滑、固定步长和变化率过稳的集体异常模式，是人工生成、过度插值或过度平滑处理的常见线索。",
    },
    "digit": {
        "label": "小数位/数字规律",
        "short": "数字",
    ...
    "physical": {
        ...
        "reason": "用可配置农业规则做有效性校验，提示超出物理范围、疑似单位错用和字段规则覆盖不足。",
    },
    "timeseries": {
        "label": "时间序列自然性",
        "short": "时序",
